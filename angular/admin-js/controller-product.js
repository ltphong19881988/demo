adminApp.controller("productCtrl", function($rootScope, $scope, $http, $compile, $routeParams, DTOptionsBuilder, DTColumnBuilder, DTColumnDefBuilder){
    ...
    if($routeParams.action != null){
        if(parseInt(jQuery('#formAddProduct div.node').eq(0).attr('status')) == 0)
        {
            let params = {
                method : 'GET', 
                url : '/admin/category/all-category?idParent=5ce6e40014a02912548dd902',
            }
            submitBackend(params, $http, function(res) {
        ...
        .withOption('ajax', {
            ...
            type: 'POST',
            data : {
                idCategoryType : '5ce6e3e814a02912548dd8fe'
            }
        })
        // .withDataProp('data')
    ...
    $scope.submitProduct = function(e){
        var params = {action: $routeParams.action, data : {_id : $routeParams.productId} };

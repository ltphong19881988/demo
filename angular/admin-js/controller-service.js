adminApp.controller("serviceCtrl", function($rootScope, $scope, $http, $compile, $routeParams, DTOptionsBuilder, DTColumnBuilder, DTColumnDefBuilder){
    ...
    $scope.listImgsPost = [];
    if($routeParams.action == "add") $rootScope.pageTitle = "Admin - Thêm mới dịch vụ";
    if($routeParams.action == "edit") {
        $rootScope.pageTitle = "Admin - Sửa dịch vụ";
        jQuery("section.card .card-title").html("Sửa dịch vụ");
        let params = {
            method : 'GET', 
    ...
    if($routeParams.action != null){
        if(parseInt(jQuery('#formAddProduct div.node').eq(0).attr('status')) == 0)
        {
            let params = {
                method : 'GET', 
                url : '/admin/category/all-category?idParent=5ce6e41014a02912548dd903',
            }
            submitBackend(params, $http, function(res) {
        ...
        $scope.dtOptionsService = DTOptionsBuilder.newOptions()
        .withOption('ajax', {
            url: '/admin/product/all-post',
            type: 'POST',
            data : {
                idCategoryType : '5ce6e3e814a02912548dd8ff'
            }
        })
        // .withDataProp('data')

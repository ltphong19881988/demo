...
router.get("/index", function(req, res, next){
    ...
    res.render("admin/commissions/index", data);
})

router.post('/all-commissions-index', function(req, res, next){
    ...
    async.parallel({
        ...
        GetCommissionsFilters(option, skip, limit, function(result){
            // console.log(result);
            res.json(result);
        })
    })
...
router.post('/count-all-commissions-index', function(req, res, next){
    var option = getOptions(req);
    async.parallel({
        checkUser : function(callback){
            checkUserFilter(option, req, function(result){
        ...
        SumCommissionsAmountFilters(option, function(result){
            if(result.length > 0){
                res.json(result[0]);
            }else{
...
function GetCommissionsFilters(option, skip, limit, callback){
...
function SumCommissionsAmountFilters(option, callback){
    Commission.aggregate([
        {
            $match: option,
        },
...
router.get('/sum-commissions-amount', function(req, res, next){
    var option = getOptions(req);
    async.parallel({
        checkUser : function(callback){
            checkUserFilter(option, req, function(result){
...
router.post('/commissions-to-excel', function(req, res, next){
